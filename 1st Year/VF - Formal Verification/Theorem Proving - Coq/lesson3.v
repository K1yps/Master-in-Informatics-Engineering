...
Proof.
  ...
  simpl.
  elim (Z.eq_dec a a). 
    + Search orb. intro. apply Bool.orb_true_r.
    + intro. contradiction.
Qed.
...
Proof.
  ...
  simpl.
  elim (Z.eq_dec z a).
  intro. Search (S _ = S _). apply eq_S. apply H. (* auto. *)
  intro. apply H. (* auto. *)
Qed.
...
Proof.
  intros. induction l.
  - simpl. destruct (Z.eq_dec z x). contradiction. reflexivity.
...
Proof.
  ...
  - destruct IHl.
    exists (insert a x).
    + apply Perm_cons with a l x in p.
