...
(* Alinea e*)
(* Quando se tenta provar a propriedade false seguinte, o TLA+ mostrará um contraexemplo com um caminho possivel para este caso *)
alineaE == \E i \in procs: [] ( pc[i] # "critical")


(*Alinea f*)
(* De momento isto ta mal, esta dizer que se tiver um processo se existir um processo na região critica e outro à espera*)
(* Então o outro eventualmente entrará na região critica, não garante que o i1 entrar a objetivamente depois do i2*)
waits == {"wait0", "wait1", "wait2"}

alineaF == \A i1 \in procs : [] (\E i2 \in procs : i1 # i2 /\ pc[i2] \in waits /\ pc[i1] = "critical" =>  /\ <>(pc[i2]="critical"))

(*
Second Attempt, Imcompleto
alineaF == \A i1 \in procs : 
                \E i2 \in procs :  i1 # i2 /\ pc[i2] \in waits /\ pc[i1]="critical"
